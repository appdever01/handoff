...
export function openStore(directory: string) {
  ...
  return {
    ...
    get(id: string): Handoff | undefined {
      const row = db.prepare("SELECT data FROM handoffs WHERE id=?").get(id) as
    ...
    save(handoff: Handoff) {
      ...
      ).run(handoff.id, handoff.creator, JSON.stringify(handoff));
    },
    list(creator: string): Handoff[] {
      return (
        db
          .prepare(
            "SELECT data FROM handoffs WHERE creator=? ORDER BY rowid DESC",
          )
          .all(creator) as { data: string }[]
      ).map((row) => JSON.parse(row.data));
    },
    session(hash: string): Session | undefined {
      const row = db
        .prepare("SELECT data FROM sessions WHERE hash=? AND expires>?")
        .get(hash, Date.now()) as { data: string } | undefined;
      return row && JSON.parse(row.data);
    },
  };
}
