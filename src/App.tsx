...
function Artwork({
  ...
  return (
    <div
      className={`artwork art-${kind} ${compact ? "compact" : ""}`}
      aria-hidden="true"
    >
      {kind === "olive" ? (
        <>
          <span className="art-corner">A MORE INTENTIONAL EVERYDAY</span>
          <div className="olive-word">
            olive<span>STUDIO</span>
          </div>
          <div className="olive-leaf leaf-one" />
          <div className="olive-leaf leaf-two" />
          <span className="art-foot">BRAND IDENTITY · 2026</span>
        </>
      ) : kind === "form" ? (
        <>
          <span className="art-corner">FORM & FIELD®</span>
          ...
          <span className="art-foot">A FRESH PERSPECTIVE.</span>
        </>
      ) : (
        <>
          <span className="art-corner">SLOW MORNINGS. GOOD COFFEE.</span>
          <div className="coffee-bag">
            <span>
              kinfolk<span className="coffee-star">✳</span>
            </span>
            <small>
              GOOD COMPANY.
              <br />
          ...
          <span className="art-foot">PACKAGING EXPLORATION · VOL. 01</span>
        </>
      )}
...
function WalletModal({
  ...
  async function signIn(currency: Currency) {
    ...
    setError("");
    try {
      connected(await connect(currency));
      close();
    } catch (error) {
...
function DraftModal({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      data = draftSchema.parse({
        ...raw,
        currency: handoff?.currency ?? user.currency,
        deadline: new Date(String(raw.deadline) + "T23:59:00").toISOString(),
      });
    } catch {
      setError(en.required);
      return;
    }
    setBusy(true);
    setError("");
    try {
      const result = await api<{ handoff: Handoff }>(
        handoff ? `/handoffs/${handoff.id}` : "/handoffs",
        { method: handoff ? "PUT" : "POST", body: JSON.stringify(data) },
      );
  ...
  return (
    <Modal title={handoff ? en.editTitle : en.newTitle} close={close}>
      <p className="muted">{en.newSubtitle}</p>
      <form onSubmit={(event) => void submit(event)} className="draft-form">
        <label>
          {en.titleLabel}
          <input
            name="title"
            ...
            required
            maxLength={100}
            defaultValue={handoff?.title}
            placeholder={en.titlePlaceholder}
          />
        </label>
        <label>
          {en.descriptionLabel}
          <textarea
            ...
            rows={2}
            maxLength={2000}
            defaultValue={handoff?.description}
            placeholder={en.descriptionPlaceholder}
          />
        </label>
        <label>
          {en.clientLabel}
          <input
            ...
            required
            maxLength={100}
            defaultValue={handoff?.clientLabel}
            placeholder={en.clientPlaceholder}
          />
        </label>
          ...
          <label>
            ...
            <div className="amount-input">
              <input
                ...
                inputMode="decimal"
                pattern="[0-9]+(\.[0-9]{1,6})?"
                defaultValue={handoff?.amount}
                placeholder="0.00"
              />
              <span>{handoff?.currency ?? user?.currency ?? "NIM"}</span>
            </div>
          </label>
          <label>
            {en.deadline}
            <input
              ...
              max={new Date(Date.now() + 29 * 86400_000).toLocaleDateString(
                "en-CA",
              )}
              defaultValue={
                handoff?.deadline.slice(0, 10) ??
                new Date(Date.now() + 7 * 86400_000).toLocaleDateString("en-CA")
              }
            />
          </label>
        </div>
        ...
        <label>
          ...
          <textarea
            ...
            rows={3}
            maxLength={2000}
            defaultValue={handoff?.terms ?? en.defaultTerms}
          />
        </label>
        <p className="field-note">{en.termsHelp}</p>
        {error && (
        ...
        <div className="modal-footer">
          ...
          <button className="button primary" disabled={busy}>
            {busy ? en.saving : user ? en.saveDraft : en.signInToSave}
            <ArrowRight size={16} />
          </button>
        </div>
      </form>
  ...
  return (
    <>
      <div className="delivery-heading">
        <div>
          <p className="eyebrow">{example ? en.sample : en.delivery}</p>
          <h1>{handoff.title}</h1>
          <p className="muted delivery-description">{handoff.description}</p>
        </div>
        {owner && handoff.status === "draft" && (
        ...
        <div className="delivery-main">
          <div className="preview-frame">
            ...
            <div className="preview-caption">
              ...
              <span>{en.protected}</span>
            </div>
          </div>
          <p className="field-note privacy-note">
            <LockKeyhole size={13} />
            {en.privateNote}
          </p>
          <section className="panel file-panel">
            <div className="section-heading">
              <h3>{en.originalFiles}</h3>
              <span className="muted">
                {handoff.files.length} {en.files}
              </span>
            </div>
            {handoff.files.map((f, index) => (
              <button
                key={f.id}
                className={`file-row ${index === selected ? "selected" : ""}`}
          ...
          <section className="panel terms-panel">
            ...
            <p className="field-note">
              {en.deadline}: {formatDate(handoff.deadline)}
            </p>
            <p className="field-note">{en.downloadWindow}</p>
          </section>
        </div>
            ...
            <section className="panel tools-panel">
              ...
              {handoff.status === "draft" ? (
                <>
                  ...
                  />
                  <button
                    disabled={busy || handoff.files.length >= 10}
                    className="button secondary full-width"
                    onClick={() => input.current?.click()}
                  >
                    <Upload size={16} />
                    {busy ? en.uploading : en.addFiles}
                  </button>
                    ...
                    <>
                      ...
                      {file.scan === "quarantined" && (
                        <button
                          disabled={busy}
                          className="button secondary full-width"
                          onClick={() => void action(`files/${file.id}/rescan`)}
                        >
                          {en.rescan}
                        </button>
                      )}
                  ...
                  <button
                    ...
                    onClick={() =>
                      void action("approve-previews", {
                        fileIds: handoff.files.map((f) => f.id),
                      })
                    }
                  >
                    <CheckCheck size={16} />
                  ...
                  <button
                    ...
                    disabled={
                      busy ||
                      !handoff.files.length ||
                      handoff.files.some((f) => !f.approved)
                    }
                  ...
                  <p className="field-note">{en.publishHelp}</p>
                </>
              ) : (
                <>
                  <button
                  ...
                  <h4>{en.invite}</h4>
                  <p className="field-note">{en.inviteHelp}</p>
                  {handoff.clientWallet ? (
                    <>
                      <p className="field-note">{en.boundWallet}</p>
                      <p className="wallet-address">{handoff.clientWallet}</p>
                    </>
                  ) : wallets.length ? (
                    wallets.map((wallet) => (
                      <div className="wallet-request" key={wallet}>
                        <p className="wallet-address">{wallet}</p>
                        <button
                          className="button secondary"
                          onClick={() => setBinding(wallet)}
                        >
                          {en.approveWallet}
                        </button>
                      </div>
                    ...
                    <p className="field-note">{en.noRequests}</p>
                  )}
                </>
              )}
            </section>
      ...
      {binding && (
        <Modal title={en.bindTitle} close={() => setBinding("")}>
          <p>{en.bindHelp}</p>
          <p className="wallet-address">{binding}</p>
          <button
            className="button primary"
            disabled={busy}
            onClick={() => void action("bind-client", { wallet: binding })}
          >
            {en.approveWallet}
          </button>
        </Modal>
...
export function App() {
  ...
  useEffect(() => {
    ...
    void api<{ handoff: Handoff }>(
      parts[1] === "draft" ? `/handoffs/${parts[2]}` : `/public/${parts[2]}`,
    )
      .then(async (data) => {
        const result =
  ...
  function update(handoff: Handoff) {
    setSelected(handoff);
    setHandoffs((items) =>
      items.some((h) => h.id === handoff.id)
        ? items.map((h) => (h.id === handoff.id ? handoff : h))
        : [handoff, ...items],
    );
  }
  function newDraft() {
            ...
            {path.startsWith("/pair") ? (
              ...
              <>
                <div className="page-heading">
                  <p className="eyebrow">{en.howItWorks}</p>
                  <h1>{en.howTitle}</h1>
                </div>
                <div className="how-grid">
                  {en.howSteps.map(([title, body], i) => (
